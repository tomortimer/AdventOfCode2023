...
namespace Day_11
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int y = 0; y < grid.Height; y++) 
            {
                if (grid.IsRowOneSymbol(y)) { grid.AddRow(y, '.'); Console.WriteLine("Expanded Row: " + y); }
            }
            //expand columns if necessary
            for(int x = 0; x < grid.Width; x++)
            {
                if (grid.IsColumnOneSymbol(x)) { grid.AddColumn(x, '.'); Console.WriteLine("Expanded Column: " + x); }
            }
